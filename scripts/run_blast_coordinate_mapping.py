...
from src.data.blast_runner import blast_card_against_representatives
...
def run(config: dict[str, Any]) -> None:
    ...
    Args:
        config: Merged config dict from load_config, with 'paths'
            ...
            blastdb_dir, blast_hits_output) and 'blast' (bin_dir,
            min_identity, min_query_coverage) sections.
    """
    paths = config["paths"]
    ...
    with open(output_path, "w", encoding="utf-8") as fh:
        json.dump([asdict(hit) for hit in hits], fh, indent=2)
